...
import { fetchAllReviews, updateReview } from '@/redux/slices/reviews/reviewSlice';
...
const UpdateReview = () => {
  ...
  // AllReviews already loads the full list - if this page is opened directly
  // (e.g. a refresh) without that list in memory yet, fetch it here too.
  useEffect(() => {
    if (!review) {
  ...
  useEffect(() => {
    if (review) {
      ...
      setPreviewImage(review.picture?.secure_url || '');
    }
  }, [review]);
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      setInputValue((values) => ({ ...values, picture: file }));
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateReview({ id, inputValues: formData }))
      .unwrap()
      .then((response) => {
        if (response?.success) {
            ...
            <div className="grid gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="grid gap-3">
                  ...
                  >
                    <span className="text-gray-500 text-sm font-medium">Click to change photo</span>
                ...
                <div className="flex items-center justify-center border rounded-lg p-2 bg-gray-50/50 min-h-[9rem]">
                  {previewImage ? (
                    ...
                    <span className="text-xs text-gray-400 italic">No photo — initials will be shown</span>
                  )}
                </div>
              </div>
